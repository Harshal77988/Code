...
class Admin_library extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model(array('country', 'state', 'footer_cms', 'social_links', 'rent_product_category', 'rent_product_subcategory'));
        ...
        $this->load->model(array('users', 'product_category', 'product_sub_category', 'product'));
        $this->load->model(array('payment'));
        $this->load->model(array('orders_summary', 'orders_details'));
        $this->load->model(array('coupon_category', 'coupon_method', 'coupon_method_tax', 'coupon_group'));
        /* Load Product model */
...
//        $this->load->view('admin', $this->data);
    ...
    function order_details($order_number) {
        ...
        // get the country and state here
    ...
    public function admin_order_details($order_id = NULL) {
        ...
        // Get the row array of the order filtered by the order number in the url.
        $sql_where = array($this->flexi_cart_admin->db_column('order_summary', 'order_number') => $order_id);
        $this->data['summary_data'] = $this->flexi_cart_admin->get_db_order_summary_row_array(FALSE, $sql_where);
        
        // $this->data['all_orders'] = $this->admin_library->demo_update_order_details($order_id);
        // Get an array of all order details related to the above order, filtered by the order number in the url.
        $sql_where = array($this->flexi_cart_admin->db_column('order_details', 'order_number') => $order_id);
        $this->data['item_data'] = $this->flexi_cart_admin->get_db_order_detail_array(FALSE, $sql_where);

        $qry = $this->db->get('order_status');
        $this->data['all_order_status'] = $qry->result_array();
        
        
        $renderTo = 'admin/order_details';

        if($this->data['summary_data']['order_type'] == '2') {
            $renderTo = 'admin/pre_order_details';
        }
    ...
    // render page and ites data pass to the view
...
//        return $view;
    ...
    function summary_discounts() {
        ...
        $pageTitle = 'Buy Sell Rent | Manage Coupons';
        $renderTo = 'admin/coupons';
    ...
    function update_discount($discount_id) {
        ...
        // set the parameters for rendering view
        $pageTitle = 'Buy Sell Rent | Update Coupon';
        $renderTo = 'admin/edit_coupon';
