...
class Standard_library extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model(array('product', 'product_category', 'product_sub_category', 'footer_cms', 'social_links', 'users', 'rent_product_category', 'rent_product_subcategory', 'coupon'));
...
//        }
    ...
    function delete_item($row_id = FALSE) {
        ...
        if($this->flexi_cart->delete_items($row_id)) {
            ...
            echo json_encode(array('content' => $mini_cart_view, 'cart_count' => count($this->flexi_cart->cart_items())));
            die;
        }
    }
    ...
    function insert_ajax_link_item_to_cart($item_id = 0) {
        ...
        foreach ($this->data['cart_items'] as $key => $cart_data) {
            $this->data['cart_items'][$key]['rent_duration'] = (!empty($this->input->post('rent_duration')) || $this->input->post('rent_duration') !== "0" ? $this->input->post('rent_duration') : '');
            $this->data['cart_items'][$key]['rent_product'] = (!empty($this->input->post('rent_id')) ? $this->input->post('rent_id') : '');
            $this->data['cart_items'][$key]['rent_start_date'] = (!empty($this->input->post('rent_start_date')) ? $this->input->post('rent_start_date') : date('d/m/Y'));
        }
    ...
    function save_cart_data() {
        ...
        // save cart data after deleting the items in cart
