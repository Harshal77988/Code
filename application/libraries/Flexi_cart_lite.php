...
class Flexi_cart_lite
{
	...
	public function cart_items($inc_discount = FALSE, $format = TRUE, $internal_value = FALSE)
	{
		...
		$cart_items = array();
                if($this->CI->flexi->cart_contents['items'])
	...
	public function item_quantity($row_id = FALSE, $inc_discount = FALSE, $format = TRUE)
	{
		...
		if (isset($this->CI->flexi->cart_contents['items'][$row_id]) && !empty($this->CI->flexi->cart_contents['items'][$row_id]))
		{                    
...
//			}
	 ...
	 */
	public function item_admin_data_status($row_id = TRUE)
	{
	...
	private function get_active_discounts($discount_type = FALSE, $target = FALSE, $format = TRUE, $internal_value = FALSE)
	{
			...
			if (isset($this->CI->flexi->cart_contents['settings']['discounts'][$discount_type][$target]))
			{
				...
				return array($target => 
					array(
						...
						'value_desc' => $target_discount['value'],
					)
				);
			}
		...
		else
		{
                    //echo 'tst';
			$discount_data = array();
                        $dataCode = $this->CI->flexi->cart_contents['settings']['discounts']['codes'];
                        foreach($dataCode as $dataVal){
                            $dataVal = $dataVal['value_discounted'];
                        }
//                        echo '<pre>', print_r($this->CI->flexi->cart_contents['settings']['discounts'][$discount_type]);die;
...
//					}
					...
					$discount_data[$discount_id]['value'] = $this->format_currency($discount['value'], $format, 2, $internal_value);
                                        $discount_data[$discount_id]['value_desc'] = $dataVal;
				}
			}
