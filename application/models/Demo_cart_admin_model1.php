...
class Demo_cart_admin_model extends CI_Model {
    ...
    function demo_update_discounts() {
        ...
        $this->session->set_flashdata('message', 'Manage coupons data updated successfully');
        redirect(current_url());
    }
...
//			$row = $this->input->post('update');
        ...
        $sql_update = array(
            $this->flexi_cart_admin->db_column('discounts', 'type') => "2", //$this->input->post('coupon_type'),
            $this->flexi_cart_admin->db_column('discounts', 'method') => "12", //$this->input->post('coupon_method'),
            $this->flexi_cart_admin->db_column('discounts', 'tax_method') => "2", //$this->input->post('tax_method'),
            $this->flexi_cart_admin->db_column('discounts', 'location') => "",
            $this->flexi_cart_admin->db_column('discounts', 'zone') => "",
            ...
            $this->flexi_cart_admin->db_column('discounts', 'recursive') => "",
            $this->flexi_cart_admin->db_column('discounts', 'non_combinable') => "",
            $this->flexi_cart_admin->db_column('discounts', 'void_reward_points') => "",
            $this->flexi_cart_admin->db_column('discounts', 'force_shipping_discount') => "",
            $this->flexi_cart_admin->db_column('discounts', 'custom_status_1') => '', //$this->input->post('custom_status'),
            $this->flexi_cart_admin->db_column('discounts', 'custom_status_2') => "",
            $this->flexi_cart_admin->db_column('discounts', 'custom_status_3') => "",
            $this->flexi_cart_admin->db_column('discounts', 'usage_limit') => $this->input->post('uses_limit'),
            $this->flexi_cart_admin->db_column('discounts', 'valid_date') => $from_date,
            $this->flexi_cart_admin->db_column('discounts', 'expire_date') => $to_date,
        ...
        if ($this->flexi_cart_admin->update_db_discount($sql_update, $discount_id)) {
            ...
            $this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
            redirect('admin_library/summary_discounts');
        } else {
            // Set errors.
            $this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
            redirect(current_url());
...
//      }
    ...
    function demo_insert_discount() {
        $sql_insert = array(
            ...
            $this->flexi_cart_admin->db_column('discounts', 'method') => 12, // $this->input->post('coupon_method'),
            $this->flexi_cart_admin->db_column('discounts', 'tax_method') => 0, //$this->input->post('tax_method'),
