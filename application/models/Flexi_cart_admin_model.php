...
//        echo 'here';die;
        $_SESSION['order_sess_id'] = $this->db->insert($this->flexi->cart_database['order_summary']['table'], $sql_insert);
...
//            echo "ss".$this->get_enabled_status('db_cart_data');die;
            if ($query->num_rows() == 1) {
                $cart_data = $query->row_array();
//                echo '<pre>', print_r($cart_data[$this->flexi->cart_database['db_cart_data']['columns']['cart_data']]);die;
    ...
    public function delete_cart_data($sql_where = FALSE, $force_delete = FALSE) {
        ...
        if (!$force_delete) {
            $sql_where['cart_data_readonly_status'] = 0;
        }
