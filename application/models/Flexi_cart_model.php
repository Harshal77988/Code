...
class Flexi_cart_model extends Flexi_cart_lite_model {
    ...
    private function get_item_data($item = FALSE, $item_data = FALSE, $insert = FALSE, $force_column_update = FALSE) {
        ...
        if (isset($item_data[$this->flexi->cart_columns['item_quantity']]) && $item_data[$this->flexi->cart_columns['item_quantity']] <= 0) {
            ...
            $item[$this->flexi->cart_columns['item_quantity']] = $this->format_calculation($item_data[$this->flexi->cart_columns['item_quantity']], $this->flexi_cart_lite_model->get_config_setting('quantity_decimals'), TRUE);
        ...
        if (isset($item_data[$this->flexi->cart_columns['item_price']]) &&
            ...
            if (!$insert && (!isset($item_data[$this->flexi->cart_columns['row_id']]) ||
                ...
                $item_internal_tax_data = $this->calculate_tax($item_data[$this->flexi->cart_columns['item_price']], $this->flexi->cart_contents['settings']['tax']['rate']);

                $item_tax_data = $this->calculate_tax($item_internal_tax_data['value_ex_tax'], $this->flexi->cart_contents['settings']['tax']['internal_rate'], FALSE, TRUE);
    ...
    private function set_item_admin_data($item, $row_id) {
        ...
        if (isset($item[$this->flexi->cart_columns['item_quantity_shipped']])) {
            $this->flexi->cart_contents['settings']['admin_data']['items'][$row_id]['item_quantity_shipped'] = ($item[$this->flexi->cart_columns['item_quantity']] > $item[$this->flexi->cart_columns['item_quantity_shipped']]) ?
        ...
        if (isset($item[$this->flexi->cart_columns['item_quantity_cancelled']])) {
            $this->flexi->cart_contents['settings']['admin_data']['items'][$row_id]['item_quantity_cancelled'] = ($item[$this->flexi->cart_columns['item_quantity']] > $item[$this->flexi->cart_columns['item_quantity_cancelled']]) ?
        ...
        $this->flexi->cart_contents['settings']['admin_data']['items'][$row_id]['stock_allocation'] = ($saved_quantity - ($saved_quantity_cancelled - $this->flexi->cart_contents['settings']['admin_data']['items'][$row_id]['item_quantity_cancelled']) -
    ...
    public function calculate_cart() {
        ...
        if ($item_total_discount_data = $this->validate_summary_discount($discount_data['item_summary_total'], $this->flexi_cart->item_summary_total(TRUE, FALSE, TRUE))) {
            ...
            if ($discount_calculation_data = $this->calculate_summary_discount($item_total_discount_data, $this->flexi_cart->cart_taxable_value(FALSE, TRUE), $this->flexi_cart->cart_non_taxable_value(FALSE, TRUE), $item_total_tax, FALSE, FALSE, 'item_total'
                    )) {
        ...
        $ship_tax_data = $this->calculate_tax($this->flexi->cart_contents['summary']['shipping_total'], $this->flexi->cart_contents['settings']['shipping']['tax_rate'], FALSE, TRUE);
        ...
        if ($shipping_discount_data) {
            ...
            if ($discount_calculation_data = $this->calculate_summary_discount($shipping_discount_data, $ship_tax_data['taxable_value'], $ship_tax_data['non_taxable_value'], $ship_tax_data['tax_value'], $this->flexi->cart_contents['settings']['shipping']['tax_rate'], TRUE, 'shipping'
                    )) {
                // Save summary discount data to cart session array.
        ...
        if ($total_discount_data = $this->validate_summary_discount($discount_data['total'], $this->flexi_cart->item_summary_total(TRUE, FALSE, TRUE))) {
            ...
            if ($discount_calculation_data = $this->calculate_summary_discount($total_discount_data, $this->flexi_cart->cart_taxable_value(FALSE, TRUE), $this->flexi_cart->cart_non_taxable_value(FALSE, TRUE), $this->flexi_cart->tax_total(TRUE, FALSE, TRUE), FALSE, FALSE, 'total'
                    )) {
                // Save summary discount data to cart session array.
        ...
        if (!empty($discount_data['reward_vouchers'])) {
            foreach ($discount_data['reward_vouchers'] as $voucher_data) {
                ...
                if ($voucher_calculation_data = $this->calculate_summary_discount($voucher_data, $this->flexi_cart->cart_taxable_value(FALSE, TRUE), $this->flexi_cart->cart_non_taxable_value(FALSE, TRUE), $this->flexi_cart->tax_total(TRUE, FALSE, TRUE), FALSE, FALSE, 'reward_vouchers'
                        )) {
    ...
    public function calculate_shipping_items($discount_data, $void_ship_discount) {
        ...
        foreach ($this->flexi->cart_contents['items'] as $row_id => $row) {
            ...
            if (!is_numeric($item_shipping_rate) && $this->flexi->cart_database['item_shipping']['table']) {
                if ($item_shipping_data = $this->flexi_cart_lite_model->get_database_item_shipping_rate($row[$this->flexi->cart_columns['item_id']], $this->flexi_cart->shipping_location_data())) {
            ...
            if (!empty($separate_item_ids) && in_array($row_id, $separate_item_ids)) {
                ...
                if ($this->get_enabled_status('shipping')) {
                    $item_shipping_rate = $this->get_database_shipping_data(
                            $this->flexi_cart->shipping_id(), $this->flexi_cart->shipping_location_data(), $row[$this->flexi->cart_columns['item_price']], $row[$this->flexi->cart_columns['item_weight']]
                    );
                }
...
//		echo '<pre>', print_r($tbl_cols_discount);die;
        $sql_select = array(
            ...
            $tbl_cols_discount['value_discounted']
        );
                        ...
                        foreach ($applicable_items as $row_id => $item_data) {
                            if ($discounted_quantity > 0) {
                                ...
                                $discount_calculation_data = $this->calculate_discount(
                                        $discount_cols['value_discounted'], $item_tax_data['taxable_value'], $item_tax_data['non_taxable_value'], $item_tax_data['tax_value'], $item_tax_rate, $discount_cols['calculation_id'], $discount_cols['tax_method']
                                );
     ...
     */
    private function calculate_summary_discount($discount_data, $taxable_value_ex_tax = 0, $non_taxable_value = 0, $tax_value = 0, $tax_rate = FALSE, $append_values = TRUE, $target_column = FALSE) {
        ...
        $discount = $this->calculate_discount(
                $discount_data['value_discounted'], $taxable_value_ex_tax, $non_taxable_value, $tax_value, $tax_rate, $discount_data['calculation_id'], $discount_data['tax_method']
        );
        ...
        if (($target_column == 'shipping' && $discount_data['calculation_id'] == 3) && ($discount['total'] > $non_discounted_total)) {
            $discount = $this->calculate_discount(
                    $non_discounted_total, $taxable_value_ex_tax, $non_taxable_value, $tax_value, $tax_rate, $discount_data['calculation_id'], $discount_data['tax_method']
            );
        }
    ...
    public function set_discount_codes($discount_codes = FALSE) {
        ...
        $discount_custom_data = $this->validate_discount_codes($discount_codes);
        if (isset($discount_codes[0])) {
            if ($discount_codes[0] === $discount_custom_data[0]['disc_code']) {
        ...
        $tbl_cols_discount = $this->flexi->cart_database['discounts']['columns'];
//		$tbl_cols_discount = array('value_discounted'=>'value_discounted');
