...
class Demo_cart_model extends CI_Model {
    ...
    function demo_insert_ajax_link_item_to_cart($item_id = 0) {
        ...
        if (isset($item_id) && $item_id != '') {
            ...
            if($this->input->post('rent_id') || !empty($this->input->post('rent_id')) || $this->input->post('rent_id') == '1') {

                $query = $this->db->select('quantity as stock_quantity')
                ->from('tbl_rent_products')
                ->where('id', $item_id)
                ->get();
                ...
                $cart_data = array(
                    ...
                    'rent_id' => '1',
                    'rent' => $this->input->post('rent_basic'),
                    'rent_duration' => $this->input->post('rent_duration'),
                    'rent_duration_val' => $this->input->post('rent_duration_val'),
                    'rent_duration_param' => $this->input->post('rent_duration_param'),
                    'rent_start_date' => $this->input->post('rent_start_date')
                );

                // echo "<pre>";
                // echo "In rent section";
                // print_r($cart_data); die;

            } else {
                
                $query = $this->db->select('category_id, quantity as stock_quantity')
                ->from('tbl_products')
                ->where('id', $item_id)
                ->get();
...
//        redirect('lite_library/item_ajax_examples');
    ...
    function demo_save_order($order_data) {
        ...
        $custom_summary_data = array(
            ...
            'ord_demo_bill_address_02' => (!empty($order_data['billing']['add_02']) ? $order_data['billing']['add_02'] : ''),
            ...
            'ord_summary_savings_total' => (!empty($order_data['discount_saving_total']) ? $order_data['discount_saving_total'] : '0'),
            'ord_tax_rate' => $order_data['ord_tax_rate'],
            'ord_tax_total' => $order_data['ord_tax_total'],
            'ord_reward_voucher_desc' => (!empty($order_data['ord_reward_voucher_desc']) ? $order_data['ord_reward_voucher_desc'] : '0'),
            'ord_shipping' => (!empty($order_data['shipping_amount']) ? $order_data['shipping_amount'] : '0'),
