...
//            $user_id = 1;
        ...
        $custom_summary_data = array(
            ...
            'ord_summary_savings_total' => $order_data['discount_saving_total'],
            'ord_reward_voucher_desc' => $order_data['ord_reward_voucher_desc'],
            'ord_shipping' => $order_data['shipping_amount']
        );
        ...
        foreach ($this->flexi_cart_admin->cart_items(TRUE, FALSE, TRUE) as $row_id => $item) {
            ...
            if (isset($item['duraturn_offer']) && $item['duraturn_offer']!='' && $item['duraturn_offer']!='false') {
                $custom_item_data[$row_id]['ord_det_discount_description'] = $item['duraturn_offer'];
            }
            if (isset($item['tire_size']) && $item['tire_size']!='' ) {
                $custom_item_data[$row_id]['ord_det_status_message'] = $item['tire_size'];
            }
        }
