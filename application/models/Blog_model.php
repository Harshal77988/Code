...
//            $fields = "b.*,lang.lang_name,(select category_name from " . $this->db->dbprefix('mst_blog_categories') . " bc where bc.category_id=b.category_id) as category_name,(select `url` from " . $this->db->dbprefix('mst_uri_map') . " u where u.rel_id=b.post_id and u.`type`='blog-post') as page_url";
	...
	//$this->db->join("mst_languages as lang","lang.lang_id=b.lang_id","left");
     ...
     public function getBlogDetails($fields = '', $condition_to_pass = '', $order_by_to_pass = '', $limit_to_pass = '', $offset = '', $debug_to_pass = 0) {
        if ($fields == '')
...
//            $fields = "b.*,u.first_name,u.last_name,u.user_name,(select category_name from " . $this->db->dbprefix('mst_blog_categories') . " bc where bc.category_id=b.category_id) as category_name,(select `url` from " . $this->db->dbprefix('mst_uri_map') . " u where u.rel_id=b.post_id and u.`type`='blog-post') as page_url";
    ...
    public function getAllBlogCategories($fields = '', $condition_to_pass = '', $order_by_to_pass = '', $limit_to_pass = '', $offset = '', $debug_to_pass = 0) {
        if ($fields == '')
//            $fields = "b.*,u.first_name,u.last_name,u.user_name,(select category_name from " . $this->db->dbprefix('mst_blog_categories') . " bc where bc.category_id=b.category_id) as category_name,(select `url` from " . $this->db->dbprefix('mst_uri_map') . " u where u.rel_id=b.post_id and u.`type`='blog-post') as page_url";
       $fields="b.*,u.first_name,u.last_name";
        $this->db->select($fields, FALSE);
    ...
    public function searchPosts_old($searchKey) {
        ...
        $this->db->from("tbl_blog_posts as b");
         $this->db->join("users as u",'u.id=b.posted_by','left');     
...
//        print_r($arr);
//        exit;
        $this->db->insert("tbl_blog_comments", $arr);
        return $this->db->insert_id();
    }

    public function update_comment($arr, $condition) {
        $this->db->update("tbl_blog_comments", $arr, $condition);
    }
    ...
    public function getPostComments($fields = '', $condition = '', $order = '', $limit = '') {
        ...
        $this->db->from("tbl_blog_comments as b");
//     $this->db->join("users as u",'u.id=b.commented_id','left');
    ...
    public function insertNewPost($arr) {
        $this->db->insert("tbl_blog_posts", $arr);
        return $this->db->insert_id();
    }

    public function updatePost($arr, $condition) {
        $this->db->update("tbl_blog_posts", $arr, $condition);
    }

    public function deletePost($arr) {
        $this->db->delete("tbl_blog_posts", $arr);
    }

    public function deletePostComment($arr) {
        $this->db->delete("tbl_blog_comments", $arr);
    }
    ...
    public function getRecentBlogs() {
        $this->db->select('b.post_id,b.post_short_description,user.first_name,user.last_name,b.blog_image,b.post_title,b.posted_on');
        $this->db->from("tbl_mst_blog_posts as b");
    ...
    /**
     * Blog archieves months
     * @author    : Harshal B <harshalb@rebelute.com>
     * @date      : 17 Jan 2018
     * @return    : get the blog archieves months
     */
    public function getBlogsArchieveMonths() {

        try {
            // get the months of published blogs
            $this->db->select('post_id, DATE_FORMAT(posted_on, "%M %Y") AS created_month');
            $this->db->from("tbl_mst_blog_posts");
            $this->db->where('status', '1');
            $this->db->group_by('created_month');
     ...
     * @author    : Harshal B <harshalb@rebelute.com>
     * @date      : 17 Jan 2018
     * @return    : get the blog archieves
     */
    public function getAllBlogsByMonths() {
