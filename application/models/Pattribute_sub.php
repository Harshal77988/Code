...
class Pattribute_sub extends MY_Model {
    ...
    public $_table = 'tbl_p_sub_attributes';
    ...
    function get_sub_attributes_at_id($attr_id = null) {
        
        $this->db->select('pts.*');
        $this->db->from('tbl_p_sub_attributes pts');
        if(isset($attr_id))
...
//        $this->db->group_by('pts.id');
        $this->db->select('ipc.*,');
        $this->db->from('tbl_it_product_sub_category ipc');
        $this->db->join('tbl_p_attributes pa', 'pa.id=ipc.p_sub_category_id', 'left');
        $this->db->join('tbl_p_sub_attributes ipas', 'pa.id=ipas.attribute_id', 'left');
...
//      die();
    ...
    function delet_attr($attrId) {
        ...
        $this->db->delete('tbl_p_sub_attributes');
    }

    function update_subattr($id, $updatData) {
        $this->db->where('id', $id);
        $this->db->update('tbl_p_sub_attributes', $updatData);
